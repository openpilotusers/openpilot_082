...
static bool handle_ml_touch(UIState *s, int touch_x, int touch_y) {
  ...
  int xs[2] = {1265, 1405};
  int ys[2] = {905, 1045};
  if ((xs[0] <= touch_x && touch_x <= xs[1]) && (ys[0] <= touch_y && touch_y <= ys[1]) && (s->status != STATUS_OFFROAD) && (s->limit_set_speed == 0) && (s->sidebar_collapsed == true)) {
...
static bool handle_SA_touched(UIState *s, int touch_x, int touch_y) {
  if (s->active_app == cereal::UiLayoutState::App::NONE) {  // if onroad (not settings or home)
    if (s->awake && s->vipc_client->connected && s->status != STATUS_OFFROAD) {  // if car started or debug mode
